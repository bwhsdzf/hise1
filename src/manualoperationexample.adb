...
begin
   ...
   for I in Integer range 0..100 loop
      ...
      if MsgAvailable then
         ...
         if ICD.CheckAuthority(Icd => ICDUnit,
                               Msg => Msg) then
            Put_line("legal source");
            ICD.SetBound(Icd   => ICDUnit,
                         Bound => I);
            Put("ICD set bound to ");
            Put(Item => I);
            New_Line;
            ICD.SetJoules(Icd => ICDUnit,
                          J   => I);
            Put("ICD set jules to I too");
            New_Line;
         else
            Put_Line("Illegal Source");
         end if;
